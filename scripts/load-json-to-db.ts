...
const promisifiedFileRead: (filename: string) => Promise<Buffer> = util.promisify(fs.readFile);
const promisifiedReadDir: (filename: string) => Promise<string[]> = util.promisify(fs.readdir);
const dbPath: string = 'mongodb://JSDaddy:jsdaddy2018@ds229909.mlab.com:29909/heroku_4k4jx5rj';

async function main(): Promise<void> {
  const spinner: ora.Ora = ora('Loading').start();
  try {
    spinner.text = 'Connect to db';
    const connection: mongodb.MongoClient = await mongodb.MongoClient.connect(dbPath, { useNewUrlParser: true });
    const db: Db = connection.db('heroku_4k4jx5rj');

    const categories: { name: string, _id: string }[] = await readJSON('output/json-categories.json');
    await loadCategories(db, categories, spinner);

    const subCategories: { name: string, _id: string, category: string }[] = await readJSON('output/json-sub-categories.json');
    await loadSubCategories(db, subCategories, spinner);

    const productsFileNames: string[] = await promisifiedReadDir(path.resolve(__dirname, 'output'));
...
async function loadCategories(db: Db, categories: { name: string, _id: string }[], spinner: ora.Ora): Promise<void> {
...
async function readJSON<T>(fileName: string): Promise<T> {
  const buffer: Buffer = await promisifiedFileRead(path.resolve(__dirname, fileName));
